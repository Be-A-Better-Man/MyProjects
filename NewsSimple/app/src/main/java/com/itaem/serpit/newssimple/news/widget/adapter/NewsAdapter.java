package com.itaem.serpit.newssimple.news.widget.adapter;
...
public class NewsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void setmDate(List<Contentlist> data) {
        this.mData = data;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof ItemViewHolder) {
            ...
            Contentlist news = mData.get(position);
            if(news == null) {
                return;
            }
            ((ItemViewHolder) holder).mTitle.setText(news.getTitle());
            ((ItemViewHolder) holder).mDesc.setText(news.getDesc());
//            Uri uri = Uri.parse(news.getImgsrc());
//            ((ItemViewHolder) holder).mNewsImg.setImageURI(uri);
            if ( news.getImageurls().size()>0){
                ImageLoaderUtils.display(mContext, ((ItemViewHolder) holder).mNewsImg, news.getImageurls().get(0).getUrl());
            }
